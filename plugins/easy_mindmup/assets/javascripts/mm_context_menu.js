(function () {
  ...
  ContextMenu.prototype.prepare = function (mapModel) {
    ...
    if ($element.length === 0) {
      $element =
          $('<div id="context-menu" class="mindmup__context_menu redmine"></div>')
  ...
  ContextMenu.prototype.changerTemplate = '\
    <ul>\
      {{#changer}}\
        <li><a href="javascript:void(0)" class="mindmup-data-value-changer {{className}} mindmup-context-submenu-item {{#previous}}icon-checked icon easy-mindmup__icon--checked disabled{{/previous}}" data-key="{{key}}" data-value="{{value}}">{{name}}</a></li>\
      {{/changer}}\
    </ul>\
  ';
