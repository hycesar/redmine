...
class RedmineResources::Charts::MonthViewBookingsChart < Redmine::Helpers::Gantt
  ...
  def render_resource_bookings_by_issue(issue, resource_bookings, options = {}, project = nil, user = nil)
    ...
    user_dayoffs = dayoffs_by(user) if user.is_a?(User) && RedmineResources.people_pro_plugin_installed?
    wrap(:lines, :div, class: 'issue-line', 'data-new-url' => new_booking_path(project, user, issue, @date_from, @date_from)) do
      ...
      render_dayoffs(user_dayoffs, options) if user_dayoffs.present?
    end
  end
  ...
  def render_dayoffs(dayoffs, options)
    options[:resource_bookings].each do |rb|
      interval = rb.interval
      selected_dayoffs = dayoffs.select do |dayoff|
        (dayoff.start_date..dayoff.end_date).overlaps?(interval)
      end

      if selected_dayoffs.present?
        selected_dayoffs.each do |dayoff|
          start_date = [dayoff.start_date, rb.start_date.to_date].max
          end_date = [dayoff.due_date, rb.end_date.to_date].min
          line(start_date, end_date, 100, nil, nil, options, dayoff)
        end
      end
    end
  end
    ...
    wrap(:lines, :div, class: 'user-line') do
      ...
      while i < user_line_map.length
        ...
        if day_plan
          ...
          if day_plan[:day] == 'dayoff'
            options[:task_bar_class] = "#{day_plan[:day]}_bar"
  ...
  def html_task_for_dayoff(params, coords, _markers, label, object)
    if coords[:bar_start] && coords[:bar_end]
      style = "left: #{coords[:bar_start]}px;"
      style << "width: #{coords[:bar_end] - coords[:bar_start] - 2}px;"
      style << "background-image: repeating-linear-gradient(135deg, rgba(0, 0, 0, 0.2), rgba(0, 0, 0, 0.2) 4px, rgba(0, 0, 0, 0) 4px, rgba(0, 0, 0, 0) 8px);"
  ...
  def user_line_map(user, resource_bookings)
    (@date_from..@date_to).map do |date|
      ...
      next nil unless scheduled_hours
        type_day = 'full_workday'
        overload = (workday_length_by(user) - scheduled_hours)
        day_workday_length = workday_length_by(user)
        workload_percent = (scheduled_hours / workday_length_by(user) * 100).round

      {
        day: type_day,
        workday_length: day_workday_length,
        overload: overload,
